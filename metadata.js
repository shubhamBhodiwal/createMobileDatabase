...
const FileCreationPath = `/home/ubuntu/homeopathy_server/RealmData/Metadata/${SchemaVersion}`;
...
const config = (version, path) => { return { path: `${path}/db.realm`, schema, schemaVersion: version, migration: () => { } } };
        ...
        for (let size = 0; size < totalCount; size = size + chunksize) {
            ...
            if (objectType === 'Book') {
                data = result.rows
                    ...
                    .map((r) => {
                        try {
                            return Object.values({ ...r, size: Math.ceil(fs.statSync(`/home/ubuntu/homeopathy_server/RealmData/Books/${SchemaVersion}/${r.book_id}-REP.realm`).size / (1024 * 1024)) });
                        }
                        catch (err) {
                            try {
                                return Object.values({ ...r, size: Math.ceil(fs.statSync(`/home/ubuntu/homeopathy_server/RealmData/Books/${r.book_id}-REP.realm`).size / (1024 * 1024)) });
                            }catch (err) {
                ...
                data = result.rows
                    .slice(size, size + chunksize)
                    .map((r) => {
                        return Object.values({ ...r, size: Math.ceil(fs.statSync(`/home/ubuntu/homeopathy_server/RealmData/MMBooks/${SchemaVersion}/${r.book_id}-MM.realm`).size / (1024 * 1024)) });
                    });
            }
            else {
                data = result.rows
...
const metadata = async () => {
    // realm = await Realm.open(config(SchemaVersion, FileCreationPath));
    // let data;
    ...
    //     objectType: 'MmJson'
    // });
    // data = await pgPool.query('select book_id from book_info where book_id in (select book_id from book);');
    // data = await writeFile({
    ...
    // where b.book_id = any('{${data.rows.map(item => item.book_id).join(',')}}');`,
    //     objectClass: Book,
    //     objectType: 'Book'
    // });
    // data = await writeFile({
    ...
    fs.rmdir(FileCreationPath + '/db.realm.management',(err) => {
        if (err) console.log(err);
        else { 
            console.log('db.realm.management ')
        }
    })
    fs.unlink(FileCreationPath+ '/db.realm.lock', (err) => {
        if (err) console.log(err);
        else { 
            console.log('db.realm.lock ')
        }
    })
    fs.unlink(FileCreationPath+ '/db.realm.note', (err) => {
        if (err) console.log(err);
        else { 
