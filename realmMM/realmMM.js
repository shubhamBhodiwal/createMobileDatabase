...
const FileCreationPath = `/home/ubuntu/homeopathy_server/RealmData/MMBooks/${SchemaVersion}`;
...
const config = (book_id,version, path) => { return { path: `${path}/${book_id}-MM.realm`, schema, schemaVersion: version, migration: () => { } } };
...
const insertBook = async (book_id) => {
    realm = await Realm.open(config(book_id, SchemaVersion, FileCreationPath))

    const result = await pgPool.query(`select book_id , mm2.remedy_id , mm2.section_id , start_pos , end_pos , 
  ...
  inner join section_mm sm  on sm.section_id = mm2.section_id  where book_id = ${book_id} order by start_pos;`);
    ...
    const bookFD = await fs_open(
        path.join("C:/homeopathy_server_mobile/HomeoData/Books-Text", `${book_id}.data`),
        "r"
    );
    const book = bookFD;
    ...
    console.log('inserted', book_id);

}

const start = async () => {
    const result = await pgPool.query('select distinct book_id from materica_medica mm2 order by book_id ;');
